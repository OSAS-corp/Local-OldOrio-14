...
//        ent.Comp.NextTickTime = _gameTiming.CurTime + ent.Comp.Interval; // Arcane-Edit
                ...
                out var blood))
        {
            ...
            var bloodProportion = blood.Volume / totalBloodstreamVolume;
            ...
            var chemTransfer = FixedPoint2.Min(
                ent.Comp.SuctionUnits * chemProportion,
                availableVolume * chemProportion);

            var totalTransferred = FixedPoint2.Zero;
            var stomachCount = FixedPoint2.New(stomachList.Count);

            foreach (var stomach in stomachList)
            {
                var bloodSolution = blood.SplitSolutionWithout(
                    bloodTransfer / stomachCount,
                    ent.Comp.ToxinReagent);

                _stomach.TryTransferSolution(stomach.Owner, bloodSolution, stomach);
                totalTransferred += bloodSolution.Volume;
                ...
                totalTransferred += chemSolution.Volume;
            }

            // Orion-Start
            if (totalTransferred > FixedPoint2.Zero && TryComp<HungerComponent>(source, out hunger))
            {
    ...
    public bool CanLatch(Entity<SlimeComponent> ent, EntityUid target)
    {
        return !(IsLatched(ent)
            ...
            || !_mobQuery.HasComp(target)
            || _latchedQuery.HasComp(target));
    }
    ...
    public void Latch(Entity<SlimeComponent> ent, EntityUid target)
    {
        ...
        _physic.SetCanCollide(ent.Owner, false, force: true, manager: fixture, body: physic);

        _audio.PlayEntity(ent.Comp.EatSound, ent, ent);
        _popup.PopupPredicted(
            Loc.GetString("slime-action-latch-success", ("slime", ent), ("target", target)),
            ent,
            ent);
    ...
    public void Unlatch(Entity<SlimeComponent> ent)
    {
        ...
        var physic = EnsureComp<PhysicsComponent>(ent.Owner);
        var fixture = EnsureComp<FixturesComponent>(ent.Owner);
        _physic.SetCanCollide(ent.Owner, true, force: true, manager: fixture, body: physic);
