...
public partial class XenobiologySystem
{
    ...
    private void DoMitosis(Entity<SlimeComponent> ent)
    {
        ...
        if (_bloodQuery.TryComp(ent, out var parentBloodstream)
            && _solutionContainer.ResolveSolution(
                ent.Owner,
                parentBloodstream.BloodSolutionName,
                ref parentBloodstream.BloodSolution,
                out var parentChem))
        {
        ...
        foreach (var s in _slimes)
        {
            if (_bloodQuery.TryComp(s, out var childBloodstream)
                && _solutionContainer.ResolveSolution(
                    s,
                    childBloodstream.BloodSolutionName,
                    ref childBloodstream.BloodSolution,
                    out var childChem))
            ...
            {
                _stomach.TryTransferSolution(s, parentStomachSolutionTransfer, stomach);
            }
        }
    ...
    public Entity<SlimeComponent>? SpawnSlime(
        EntityUid parent,
        [ForbidLiteral] EntProtoId newEntityProto,
        ProtoId<BreedPrototype> selectedBreed)
    {
        ...
        var newEntityUid = PredictedSpawnNextToOrDrop(
            newEntityProto,
            parent,
            null,
            newBreed.Components);
